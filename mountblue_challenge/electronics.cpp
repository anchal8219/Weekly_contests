 ...
 */
int getMoneySpent(vector<int> keyboards, vector<int> usbDrives, int budget) {
    int maxCost = -1;
    
    sort(keyboards.begin(), keyboards.end(), greater<int>());
    sort(usbDrives.begin(), usbDrives.end(), greater<int>());
    
    // Find the maximum cost
    for (int i = 0; i < keyboards.size(); ++i) {
        for (int j = 0; j < usbDrives.size(); ++j) {
            int cost = keyboards[i] + usbDrives[j];
            if (cost <= budget) {
                maxCost = max(maxCost, cost);
                break; // Break as we have found the maximum cost for this keyboard
            }
        }
    }
